function MT_localizer_old2( Subj, Durat, Emulate )
% MT_localizer2
%
...
%
% removed task, need to edit to collect triggers
%
...
if Emulate 
    ...
    Parameters.Instruction='Please fixate at all times!  Wait for collection of Dummy volumes.';
end
[Parameters.Session Parameters.Session_name]=CurrentSession([Subj '_MTloc']); % Determine current session
...
Start_of_Expmt = GetSecs;   % Time when cycling starts
...
%     while CurrTime<=countCycle*CycleDuration 
%     if mod((round((CurrTime)/2)),floor(CycleDuration))==0
%     for CurrTime = 1:30
...
%     eval(['save ', Parameters.Session_name, ' Results Behaviour triggers']);
    eval(['save ', Parameters.Session_name, 'triggers']);
